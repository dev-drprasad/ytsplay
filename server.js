...
const DELUGE_UI_URL = `http://${process.env.DELUGE_UI_HOST ||
  "localhost:8112"}/json`;
...
class YTS {
  constructor(username, password) {
    ...
    this.cookies = null;
    this.auth();
    setTimeout(this.auth.bind(this), 2 * 60 * 60 * 1000);
  }

  auth() {
    const loginURL = new URL("https://yts.am/ajax/login");
    fetch(loginURL, {
      method: "POST",
      headers: {
      ...
      body: `username=${this.username}&password=${this.password}&_token=`,
    })
      .then(res => {
...
const yts = new YTS(process.env.YTS_USERNAME, process.env.YTS_PASSWORD);
...
app.get("/api/search", function(req, res) {
  ...
  if (type === "shows") {
    ...
    fetch(searchUrl.toString())
      ...
      .then(html => {
        ...
        const searchListElems = $(
          ".main-content .movies-list div[data-movie-id] > a"
        );
        const searchList = searchListElems
          .map((_, el) => {
            const $el = $(el);
            return {
  ...
  if (type === "movies") {
    ...
    fetch(searchUrl.toString())
      ...
      .then(jsonResponse => {
        res.json(
          (jsonResponse.data || []).map(({ title, url, year, img }) => ({
            name: title,
            url,
            year,
            imageUrl: changeHost(img),
          }))
        );
      })
      .catch(err => {
...
app.get("/api/add", (req, res) => {
  ...
  const cookieMatch =
    req.headers.cookie && req.headers.cookie.match(/_session_id=\w+/);
  if (!cookieMatch)
    return res.json({ error: { message: "Not authenticated", code: 1 } });

  fetch(url, { headers: { Cookie: yts.cookies } })
    ...
    .then(html => {
      ...
      let magnetLink = null;
      if (type === "movies") {
        magnetLink = $("#modal-quality-720p ~ a.magnet")
          .first()
          .attr("href");
      }
  ...
  if (cookieMatch) {
    fetch(DELUGE_UI_URL, {
      ...
      .then(jsonResponse => {
        ...
        const torrents = Object.values(
          (jsonResponse.result || {}).torrents || {}
        );
        return Promise.all(
          torrents.map(torrent =>
            fs
              .readFile(`/tmp/${torrent.hash}`, "utf8")
              .then(streamNo => ({ ...torrent, streamNo: Number(streamNo) }))
              .catch(() => torrent)
          )
        );
      })
...
app.post("/api/remove", (req, res) => {
  ...
  if (cookieMatch) {
    fetch(DELUGE_UI_URL, {
      ...
      .then(jsonResponse => {
        return fs
          .readFile(path.join("/tmp", hash), "utf8")
          .then(streamNo =>
            fs.unlink(path.join("/stream", `stream${streamNo}`))
          )
...
app.post("/api/stream", (req, res) => {
  ...
  fetch(DELUGE_UI_URL, {
    ...
    .then(jsonResponse => {
      const mediaFilePath = Object.entries(
        Object.values(jsonResponse.result.contents)[0].contents
      ...
      if (mediaFilePath) {
        ...
        fs.readdir("/stream")
          .then(filenames => {
            const bookedStreams = filenames
              .map(filename => filename.slice(-1)[0])
              .filter(Boolean)
              .map(Number);
            let streamNo = 1;
          ...
          .then(streamNo => {
            fs.symlink(
              path.join(process.env.DOWNLOAD_DIR, mediaFilePath),
              path.join("/stream", `stream${streamNo}`)
            )
              .then(() => {
...
app.get("/assets/*", (req, res) => {
  res.type("image/jpeg");
  fetch(`https://img.yts.am${req.path}`).then(response =>
...
app.get("/t/p/*", (req, res) => {
  res.type("image/jpeg");
  fetch(`https://image.tmdb.org${req.path}`).then(response =>
    response.body.pipe(res)
  );
});
